...
const (
	...
	ctrl              = "default:deployment/helloworld"
	container         = "greeter"
	ns                = "default"
	...
	anotherCtrl      = "another:deployment/service"
...
func TestDaemon_ListWorkloadsWithOptions(t *testing.T) {
	...
	t.Run("filter id", func(t *testing.T) {
		s, err := d.ListWorkloadsWithOptions(ctx, v11.ListWorkloadsOptions{
			Namespace: "",
			Workloads: []flux.ResourceID{flux.MustParseResourceID(ctrl)}})
		if err != nil {
			t.Fatalf("Error: %s", err.Error())
		}
		if len(s) != 1 {
	...
	t.Run("filter unsupported id kind", func(t *testing.T) {
		_, err := d.ListWorkloadsWithOptions(ctx, v11.ListWorkloadsOptions{
			Namespace: "foo",
			Workloads: []flux.ResourceID{flux.MustParseResourceID("default:unsupportedkind/goodbyeworld")}})
		if err == nil {
			t.Fatal("Expected error but got nil")
		}
	})
...
func TestDaemon_ListImagesWithOptions(t *testing.T) {
	...
	// Service 1
	svcID, err := flux.ParseResourceID(ctrl)
	...
	// Service 2
	anotherSvcID, err := flux.ParseResourceID(anotherCtrl)
	assert.NoError(t, err)
	...
	tests := []struct {
		...
		},
		{
			...
			opts: v10.ListImagesOptions{Spec: update.ResourceSpec(ctrl)},
			expectedImages: []v6.ImageStatus{
				{
					ID: svcID,
					Containers: []v6.Container{
...
func TestDaemon_Automated(t *testing.T) {
	...
	w.ForImageTag(t, d, ctrl, container, "2")
}
...
func mockDaemon(t *testing.T) (*Daemon, func(), func(), *cluster.Mock, *mockEventWriter, func(func())) {
	...
	singleService := cluster.Workload{
		ID: flux.MustParseResourceID(ctrl),
		Containers: cluster.ContainersOrExcuse{
			Containers: []resource.Container{
				{
					Name:  container,
